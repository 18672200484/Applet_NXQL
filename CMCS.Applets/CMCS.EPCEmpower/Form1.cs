    ...
    public partial class Form1 : Form
    {
        ...
        private void btnOCRwer_Click(object sender, EventArgs e)
        {
            if (!this.IsConnectRwer)
            {
                ...
                //}
                //else
                //    MessageBox.Show("发卡器连接失败", "操作提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
                if (this.Rwer.OpenCom(Convert.ToInt32(cmbCom.Text.Replace("COM", "")), 20))
                {
                    this.IsConnectRwer = true;
                }
                else
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            if (this.Rwer.WriteEPC(newTagId))
            {
                if (rbtnNetMode.Checked)
                {
                    ...
                    {
                        this.oracleHelper.ExecuteNonQuery(this.selfAppConfig.InsertSQL, new OracleParameter[] { new OracleParameter("Id", Guid.NewGuid().ToString()), new OracleParameter("CardNumber", newTagId.Substring(19)), new OracleParameter("TagId", newTagId), new OracleParameter("InStorageDate", DateTime.Now.ToString()) });
                    }
                    else
                    {
                        MessageBox.Show("入库失败，该标签号已存在！", "操作提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                //if (MessageBox.Show("授权成功，标签号：" + newTagId + "\r\n是否继续授权？", "操作提示", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                //{
                MessageBox.Show("授权成功，标签号：" + newTagId + "\r\n请放入下一张标签卡，已授权的标签卡距离不要太近！", "操作提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
