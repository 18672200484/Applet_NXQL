	...
	public class PrintWeightReport
	{
		...
		private void _PrintDocument_PrintPage(object sender, PrintPageEventArgs e)
		{
			...
			if (this._BuyFuelTransport != null)
			{
				...
				g.DrawString("日  期：" + this._BuyFuelTransport.TareTime.ToString("yyyy-MM-dd HH:mm"), ContentFont, Brushes.Black, LeftValue, TopValue);
				TopValue += 24;

				g.DrawLine(new Pen(Color.Black, 2), 0, TopValue, 300 - LeftValue, TopValue);
				TopValue += 15;

				g.DrawString("单  号：" + this._BuyFuelTransport.SerialNumber, ContentFont, Brushes.Black, LeftValue, TopValue);
				TopValue += 24;

				g.DrawString("矿  点：", ContentFont, Brushes.Black, LeftValue, TopValue);
				...
				g.DrawString("车  号：" + this._BuyFuelTransport.CarNumber, ContentFont, Brushes.Black, LeftValue, TopValue);
				TopValue += 24;
				...
				g.DrawString(string.Format("毛  重：{0} 吨", this._BuyFuelTransport.GrossWeight), ContentFont, Brushes.Black, LeftValue, TopValue);
				TopValue += 24;

				g.DrawString(string.Format("皮  重：{0} 吨", this._BuyFuelTransport.TareWeight), ContentFont, Brushes.Black, LeftValue, TopValue);
				TopValue += 24;

				g.DrawString(string.Format("净  量：{0} 吨", this._BuyFuelTransport.SuttleWeight), ContentFont, Brushes.Black, LeftValue, TopValue);
				TopValue += 24;

				g.DrawString(string.Format("扣  吨：{0} 吨", this._BuyFuelTransport.DeductWeight), ContentFont, Brushes.Black, LeftValue, TopValue);
				TopValue += 24;

				g.DrawString("备  注：" + this._BuyFuelTransport.Remark, ContentFont, Brushes.Black, LeftValue, TopValue);
				TopValue += 24;
