...
namespace CMCS.CarTransport.QueueScreen.Frms.Sys
{
	public partial class FrmQueueScreen : Form
	{
		...
		private void LoadContent()
		{
			...
			List<CmcsBuyFuelTransport> list = commonDAO.SelfDber.Entities<CmcsBuyFuelTransport>("where IsFinish=0 and trunc(InFactoryTime)=trunc(sysdate) order by CreationTime ");
		...
		{
			if (panelIn.Height <= panelOut.Height)
				return true;
