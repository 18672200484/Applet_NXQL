	...
	public partial class FrmQueuer : DevComponents.DotNetBar.Metro.MetroForm
	{
							...
							if (this.CurrentAutotruck.IsUse == 1)
							{
								...
								if (unFinishTransport != null)
								{
									...
									if (this.CurrentImperfectCar.IsFromDevice)//有未完成的运输记录直接入厂
									{
										...
										{
											if (queuerDAO.SaveBuyFuelTransport(unFinishTransport.TransportId, DateTime.Now))
											{
										...
										else if (unFinishTransport.CarType == eCarType.其他物资.ToString())
										{
		...
		bool SaveBuyFuelTransport()
		{
			...
			try
			{
				...
				if (queuerDAO.JoinQueueBuyFuelTransport(this.CurrentAutotruck, this.SelectedMine_BuyFuel, this.SelectedTransportCompany_BuyFuel, this.SelectedFuelKind_BuyFuel, this.CurrInNetTransport, (decimal)txtTicketWeight_BuyFuel.Value, this.CurrentSampler, this.CurrentWeighter, DateTime.Now, txtRemark_BuyFuel.Text, CommonAppConfig.GetInstance().AppIdentifier, this.SelectedSupplier_BuyFuel))
				{
			...
			try
			{
				switch (this.CurrentFlowFlag)
				{
					case eFlowFlag.匹配调运:
						...
						this.CurrInNetTransport = Dbers.GetInstance().SelfDber.Entity<CmcsInNetTransport>("where CarNumber=:CarNumber and (StepName='矿发' or StepName='在途') and trunc(StartTime)>trunc(sysdate-4) order by creationtime desc", new { CarNumber = this.CurrentAutotruck.CarNumber });
					...
					case eFlowFlag.等待离开:
						...
						// 当前道路地感无信号时重置
						if (this.InductorCoil1State && !HasCarOnCurrentWay()) ResetBuyFuel();
		...
		bool SaveGoodsTransport()
		{
			...
			try
			{
				...
				if (queuerDAO.JoinQueueGoodsTransport(this.CurrentAutotruck, this.SelectedSupplyUnit_Goods, this.SelectedGoodsType_Goods, ((DataItem)cmbMineName_GoodsType.SelectedItem).Value, ((DataItem)cmbUpload_GoodsType.SelectedItem).Value, ((DataItem)cmbFuelName_GoodsTye.SelectedItem).Value, DateTime.Now, "", CommonAppConfig.GetInstance().AppIdentifier, ref goodstransport))
				{
					LetPass();
			...
			try
			{
				switch (this.CurrentFlowFlag)
				{
					...
					case eFlowFlag.等待离开:
						...
						if (this.InductorCoil1) this.InductorCoil1State = true;
						// 当前道路地感无信号时重置
						if (this.InductorCoil1State && !HasCarOnCurrentWay()) ResetGoods();
		...
		Pen greenPen3 = new Pen(Color.Lime, 2);
		...
		private void panCurrentCarNumber_Paint(object sender, PaintEventArgs e)
		{
			try
			{
				...
				int height = 12;
				// 绘制地感1
				e.Graphics.DrawLine(this.InductorCoil1 ? redPen3 : greenPen3, 15, 6, 15, panel.Height - height);
