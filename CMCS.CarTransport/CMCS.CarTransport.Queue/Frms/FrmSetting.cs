...
namespace CMCS.CarTransport.Queue.Frms
{
	public partial class FrmSetting : DevComponents.DotNetBar.Metro.MetroForm
	{
		...
		void InitForm()
		{
			InitNumberAscComboBoxs(1, 30, cmbip_Rwer1Port);
		...
		bool SaveAppConfig()
		{
			...
			commonDAO.SetCommonAppletConfig("启用在途监控", Convert.ToInt16(chkOnWayMonitor.Checked).ToString());
			...
			commonDAO.SetCommonAppletConfig("采样机编号", txtSamplerCodes.Text);
			commonDAO.SetCommonAppletConfig("汽车衡编号", txtWeighterCodes.Text);
			...
			commonDAO.SetAppletConfig("读卡器1_功率", (cmbip_Rwer1Port.SelectedItem as DataItem).Value);
			commonDAO.SetAppletConfig("读卡器2_串口", (cmbRwer2.SelectedItem as DataItem).Value);
			commonDAO.SetAppletConfig("读卡器2_功率", (cmbip_Rwer1Port.SelectedItem as DataItem).Value);
