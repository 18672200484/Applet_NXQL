...
namespace CMCS.CarTransport.Queue.Frms.Transport.GoodsTransport
{
	public partial class FrmGoodsTransport_List : MetroAppForm
	{
		...
		private void btnSearch_Click(object sender, EventArgs e)
		{
			...
			if (this.cmbCarType.Text != "全部") this.SqlWhere += " and CarType = '" + this.cmbCarType.Text + "'";
		...
		private void btnReportExport_Click(object sender, EventArgs e)
		{
			try
			{
				...
				for (int i = 0; i < CurrExportData.Count; i++)
				{
					...
					Mysheet1(sheetl, i + 1, 6, entity.SuttleWeight.ToString());
					Mysheet1(sheetl, i + 1, 7, entity.SecondTime.Year < 2000 ? "" : entity.SecondTime.ToString("yyyy-MM-dd HH:mm:ss"));
				}
