...
namespace CMCS.CarTransport.DAO
{
	...
	public class CarTransportDAO
	{
		...
		public CmcsInFactoryBatch HasInFactoryBatch(CmcsBuyFuelTransport buyFuelTransport)
		{
			...
			return SelfDber.Entity<CmcsInFactoryBatch>("where Batch like '%-'|| to_char(:CreationTime,'YYYYMMDD') ||'-%' and MineId=:MineId and FuelKindId=:FuelKindId and BatchCreateType=1 and IsDeleted=0", new { CreationTime = dt, MineId = buyFuelTransport.MineId, FuelKindId = buyFuelTransport.FuelKindId });
		}
		...
		{
			return SelfDber.Entity<CmcsUnFinishTransport>("where AutotruckId=:AutotruckId and CarType=:CarType order by CreationTime desc", new { AutotruckId = autotruckId, CarType = carType });
		}
